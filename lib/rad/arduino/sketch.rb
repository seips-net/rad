...
class Rad::Arduino::Sketch
  ...
  # however, it only knows about long, float, str, byte
  ...
  def define(arg)
    if arg
        ...
        if value =~ /^-(\d|x)*$/ # negative
          type = "long"
        elsif value =~ /^-(\d|\.|x)*$/ # negative float
          type = "float" 
        elsif value =~ /[a-zA-Z]/
          type = "str"
          value = "\"#{value}\""
          ...
          raise ArgumentError, "could not determine the define type, got #{value}"
        end
...
# #todo clean this up
  def compose_setup #:nodoc: also composes headers and signatures
    ...
    #external_vars
    external_vars << comment_box( 'plugin external variables' )
    ...
    external_vars << comment_box( 'sketch external variables' )    
    ...
    external_vars << comment_box( 'variable and accessors' )
    @declarations.each {|dec| external_vars << dec}  
    @accessors.each {|ac| external_vars << ac}

    # fix naming
    external_vars << comment_box( 'assembler declarations' )
    ...
    external_vars << comment_box( 'setup' )
    setup << "void setup() {"
    setup << "\t// pin modes"
    ...
    main << comment_box( 'main() function' )
    main << <<-CODE
int main() {
\tinit();
\tsetup();
...
CODE
    ...
    main << comment_box( 'loop! Autogenerated by RubyToC' )
  ...
  def self.post_process_ruby_to_c_methods(e)      
    clean_c_methods = []
      ...
      if e !~ /^\s*(#{C_VAR_TYPES.join('|')})(\W{1,6}|\(unsigned\()(#{$external_var_identifiers.join("|")})/ || $external_var_identifiers.empty?
